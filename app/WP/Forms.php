...
use Quick\Annotations\FormRoute;
...
class Forms {
    ...
    public function __construct() {
        $directory = getQuickPluginDirectory() . 'app/Forms';
        ...
        foreach ($classes as $class) {
            ...
            if ('php-annotations' === QUICK_SYSTEM && method_exists($reflectionClass, 'getAttributes')) {
    ...
    private function formWrapper(): void {
        ...
        if (null === $action) {
            wp_die(
                __('No action specified', 'quick')
            );
        }

        if (!array_key_exists($action, $this->actions)) {
            wp_die(__('Action not registered', 'quick'));
        }
        ...
        if (!isset($privateAction['availability'], $privateAction['controller'], $privateAction['controllerMethod'])) {
            wp_send_json_error(
                __('A controller, method and availability was not found for this action', 'quick')
            );
        }
        ...
        if (!method_exists($privateAction['controller'], $privateAction['controllerMethod'])) {
            wp_die(__('A method was not found for this action', 'quick'));
        }
